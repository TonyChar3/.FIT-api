...
const logOutUser = asyncHandler(async (req, res, next) => {
  try {
    req.logOut((err) => {
      ...
      res.status(200).cookie("connect.sid", "", {
        expires: new Date(0),
        httpOnly: true,
        path: "/",
      });
...
const anonymousUser = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (verification.error) {
      ...
      if (hashToken) {
        if (host == "dev") {
          res.status(200).cookie("fit-customer", tokenObject.token, {
          ...
          res.status(200).cookie("fit-hash", hashToken, {
            expires: new Date(2030, 0),
            maxAge: 48 * 60 * 60 * 1000,
            httpOnly: true,
            path: "/",
        ...
        res.status(200).json({ success: "newly set" });
        return;
      } else {
        throw new Error("Unable to create new acces token");
      }
    }
    res.status(200).json({ success: "already set" });
  } catch (err) {
    console.log(err);
    next(err);
  }
...
const clearSessionCookies = asyncHandler(async (req, res, next) => {
  try {
    res.status(200).cookie("fit-hash", "", {
      expires: new Date(0),
      httpOnly: true,
      path: "/",
    });
    res.status(200).cookie("fit-customer", "", {
      expires: new Date(0),
      httpOnly: true,
      path: "/",
    });
