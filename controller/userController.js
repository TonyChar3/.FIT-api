...
const anonymousUser = asyncHandler( async(req,res,next) => {
    try{
        ...
        if(verification.error){
            ...
            if(hashToken){
                res.status(200).cookie('fit-customer', tokenObject.token, { maxAge: 48 * 60 * 60 * 1000, httpOnly: true, sameSite: 'none', secure: false })
                res.status(200).cookie('fit-hash', hashToken, { maxAge: 48 * 60 * 60 * 1000, httpOnly: true, sameSite: 'none', secure: false })
