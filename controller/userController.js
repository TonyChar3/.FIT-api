...
const anonymousUser = asyncHandler( async(req,res,next) => {
    try{
        ...
        if(verification.error){
            console.log('Will set new user cookies...')
            ...
            if(hashToken){
                res.status(200).cookie('fit-customer', tokenObject.token, { maxAge: 48 * 60 * 60 * 1000, domain:'.tony-char3.com', httpOnly: true, path:'/', sameSite: 'none', secure: true })
                res.status(200).cookie('fit-hash', hashToken, { maxAge: 48 * 60 * 60 * 1000, domain:'.tony-char3.com', httpOnly: true, path:'/', sameSite: 'none', secure: true })
...
const currentUser = asyncHandler( async(req,res,next) => {
    if(req.isAuthenticated() && req.user){
        ...
        res.status(200).json({ message: 'New customer' });
    }
});
